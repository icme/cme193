	...
	*****
	...
	def __init__(self, data=None, shape=None, dtype='f'):
		...
		if (shape is None) and (data is None):
			raise ValueError('Need one of data or shape')

		if data is not None and type(data[0]) is list:
			self._shape = len(data), len(data[0])

			assert all(len(row) == self._shape[1] for row in data)

			if (shape is not None) and (shape != self._shape):
				raise ValueError('Bad shapes')

			self._data = array.array(dtype, [x for row in data for x in row])
		else:
			self._shape = (len(data), 1) if shape is None else shape

			data = [0] * (shape[0] * shape[1]) if data is None else data

			assert self._shape[0] * self._shape[1] == len(data)
	...
	def __getitem__(self, indices):
		...
		assert i < self._shape[0]
		assert j < self._shape[1]

		return self._data[i * self._shape[1] + j]
